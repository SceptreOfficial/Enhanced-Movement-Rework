class CfgVehicles {
	class All;
	class NonStrategic;
	class Wall_F;
	class Fence;

	class Rope : All {
		EMR_blacklist = 1;
	};
	class Land_WiredFence_01_16m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_WiredFence_01_4m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_WiredFence_01_8m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_Mil_WiredFenceD_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_Mil_WiredFence_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_New_WiredFence_10m_Dam_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_New_WiredFence_10m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_New_WiredFence_5m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_NetFence_03_m_9m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_IndFnc_9_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_IndFnc_3_Hole_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_NetFence_03_m_3m_hole_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_NetFence_03_m_3m_d_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_IndFnc_3_D_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_NetFence_03_m_3m_corner_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_IndFnc_Corner_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_NetFence_03_m_3m_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_IndFnc_3_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_CamoConcreteWall_01_l_4m_d_v1_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_CamoConcreteWall_01_l_4m_d_v2_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_CamoConcreteWall_01_l_4m_v1_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_CamoConcreteWall_01_l_4m_v2_F : Wall_F {
		EMR_blacklist = 1;
	};
	class Land_Razorwire_F : NonStrategic {
		EMR_blacklist = 1;
	};
	class Fort_RazorWire : NonStrategic {
		EMR_blacklist = 1;
	};
	class WireFence : NonStrategic {
		EMR_blacklist = 1;
	};
	class Wire : Fence {
		EMR_blacklist = 1;
	};
};
